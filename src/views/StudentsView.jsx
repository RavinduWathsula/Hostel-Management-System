    ...
    <div className="space-y-6 animate-fade-in">
      ...
      {isDeleteModalOpen && studentToDelete && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative bg-slate-900 border border-slate-700/50 shadow-2xl rounded-3xl w-full max-w-md overflow-hidden animate-in fade-in zoom-in-95 duration-200">
            {/* Background glowing effects */}
            <div className="absolute -top-24 -right-24 w-64 h-64 bg-rose-500/10 rounded-full blur-3xl pointer-events-none"></div>
            ...
            <div className="relative p-8 text-center flex flex-col items-center">
              {/* Icon Container */}
              <div className="w-16 h-16 rounded-2xl bg-rose-500/10 border border-rose-500/20 flex items-center justify-center text-rose-500 mb-6 shadow-inner relative overflow-hidden group">
                <div className="absolute inset-0 bg-rose-500/20 transform translate-y-full group-hover:translate-y-0 transition-transform duration-300"></div>
                <Trash2 className="w-8 h-8 relative z-10" />
              </div>
              
              <h3 className="text-2xl font-black text-white mb-2 tracking-tight">Remove Resident</h3>
              <p className="text-slate-400 text-sm mb-8 max-w-sm leading-relaxed">
                Are you absolutely sure you want to permanently remove <span className="text-rose-400 font-bold px-1">{studentToDelete.full_name}</span>? This action cannot be undone and will delete all associated data.
              </p>
              ...
              <div className="flex w-full gap-3">
                <button
                  ...
                  disabled={isDeleting}
                  className="flex-1 px-4 py-3 rounded-xl bg-slate-800 text-slate-300 font-bold text-sm hover:bg-slate-700 hover:text-white transition-all shadow-md"
                >
                  Cancel
                </button>
                <button
                  onClick={async () => {
                    setIsDeleting(true);
                    await onDeleteStudent(studentToDelete.student_id || studentToDelete.id);
                    setIsDeleting(false);
                    setIsDeleteModalOpen(false);
                    setStudentToDelete(null);
                  }}
                  disabled={isDeleting}
                  className="flex-1 px-4 py-3 rounded-xl bg-rose-600 hover:bg-rose-500 text-white font-bold text-sm shadow-lg shadow-rose-600/25 transition-all flex items-center justify-center gap-2 hover:scale-[1.02]"
                >
                  {isDeleting ? (
